...
class _InteractionDetailPageState extends State<InteractionDetailPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (_interaction != null) {
        ...
        final vehicleData = await _service.getCoreVehicleWithCustomer(_interaction!.vehicleId);
        if (vehicleData != null) {
          _vehicle = CoreVehicle.fromJson(vehicleData);
        ...
        if (_vehicle != null) {
          _historyJobs = await _service.getMaintenanceJobsByCoreVehicleId(_vehicle!.vehicleId);
        }
      }
    } catch (e) {
      _error = e.toString();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Row(
        children: [
          // Left Sidebar - Driver, CoreVehicle, Notes
          SizedBox(
            width: 350,
  ...
  Widget _buildCoreVehicleSection() {
    return Padding(
      padding: const EdgeInsets.all(16),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
