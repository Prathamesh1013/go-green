...
class _ComplianceDashboardPageState extends State<ComplianceDashboardPage> {
  ...
  Future<_ComplianceData> _loadData() async {
    final vehicles = await _supabaseService.getCoreVehicles();
        ...
        body: FutureBuilder<_ComplianceData>(
          ...
          builder: (context, snapshot) {
            ...
            return SingleChildScrollView(
              ...
              const SizedBox(height: 16),
              _buildCoreVehicleDocumentList(context, data),
                ],
              ),
            );
          },
  ...
  _ComplianceAnalytics _buildAnalytics(_ComplianceData data) {
    final docsByCoreVehicle = <String, List<ComplianceDocument>>{};
    for (final doc in data.documents) {
      docsByCoreVehicle.putIfAbsent(doc.vehicleId, () => []).add(doc);
    }

    int compliantCoreVehicles = 0;
    for (final vehicle in data.vehicles) {
      final vehicleDocs = docsByCoreVehicle[vehicle.vehicleId] ?? [];
    ...
    return _ComplianceAnalytics(
      compliantCoreVehicles: compliantCoreVehicles,
      totalCoreVehicles: data.vehicles.length,
      summary: summary,
  ...
  Widget _buildComplianceScoreCard(BuildContext context, _ComplianceAnalytics analytics) {
    final compliant = analytics.compliantCoreVehicles;
    final total = analytics.totalCoreVehicles;
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Text(
                'Valid > 30 days Â· Expiring â‰¤ 30 days Â· Expired < today',
  ...
  Widget _buildCoreVehicleDocumentList(BuildContext context, _ComplianceData data) {
    final docsByCoreVehicle = <String, List<ComplianceDocument>>{};
    for (final doc in data.documents) {
      docsByCoreVehicle.putIfAbsent(doc.vehicleId, () => []).add(doc);
    }
        ...
        children: [
          ...
          ...data.vehicles.map((vehicle) {
            ...
            return Padding(
              ...
              child: _CoreVehicleExpandableCard(
                vehicle: vehicle,
                isCompliant: isCompliant,
                needsAttentionCount: needsAttention,
                attentionDocs: attentionDocs,
                onViewAll: () => _showCoreVehicleDocumentsDialog(context, vehicle, docMap),
              ),
            );
          }),
        ],
  ...
  void _showCoreVehicleDocumentsDialog(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    ElevatedButton.icon(
                      ...
                      label: const Text('Edit CoreVehicle'),
                    ),
                  ],
                ),
              ),
  ...
  List<Widget> _buildDocumentItems(BuildContext context, Map<String, ComplianceDocument> docMap, CoreVehicle vehicle) {
  ...
  Widget _buildWarrantySection(BuildContext context, Map<String, ComplianceDocument> docMap, CoreVehicle vehicle) {
  ...
  void _showUploadDocumentDialog(BuildContext context, CoreVehicle vehicle, [String? preselectedDocType]) {
  ...
  // Add warranty dialog
  Future<void> _showAddWarrantyDialog(BuildContext context, CoreVehicle vehicle) async {
    String warrantyName = '';
    DateTime? selectedExpiryDate;
    PlatformFile? selectedFile;
    bool isUploading = false;
...
class _ComplianceData {
  final List<CoreVehicle> vehicles;
  final List<ComplianceDocument> documents;
  late final Map<String, CoreVehicle> vehicleIndex;
...
class _ComplianceAnalytics {
  final int compliantCoreVehicles;
  final int totalCoreVehicles;
  ...
  _ComplianceAnalytics({
    required this.compliantCoreVehicles,
    required this.totalCoreVehicles,
...
class _CoreVehicleExpandableCard extends StatefulWidget {
  final CoreVehicle vehicle;
  ...
  const _CoreVehicleExpandableCard({
    required this.vehicle,
  ...
  @override
  State<_CoreVehicleExpandableCard> createState() => _CoreVehicleExpandableCardState();
}

class _CoreVehicleExpandableCardState extends State<_CoreVehicleExpandableCard> {
